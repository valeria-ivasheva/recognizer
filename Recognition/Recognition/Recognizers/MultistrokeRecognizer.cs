...
{
    public class MultistrokeRecognizer : IRecognizer
    {
        ...
        private double CloudDistance(List<Point> pointsOne, List<Point> pointsTwo)
        {
            ...
            for (int i  = 0; i < pointsOne.Count; i++)
            {
                ...
                {
                    var dist = Distance.EuclideanDistance(pointsOne[i], pointsTwo[j]);
                    min = dist < min ? dist : min;
                }
                result += min;
            }
            return (result / countPoints);
        }
